...
6. `category1`, `category2`, etc.: These represent increasingly broader categories that the metric falls under, forming a hierarchy. The numbering goes from more specific (category1) to more general (category2, category3, etc.). For example:

                    -`category1`: "Growth (y/y)" indicates this is a year-over-year growth metric.

                    -`category2`: "Income Statement ($M)" shows this is part of the income statement, measured in millions of dollars.


7. `description`: A human-readable sentence summarizing the key information about this data point, including its metric, categories, date, value, and source spreadsheet.
8. `last_updated`: A timestamp indicating when this data was last refreshed or updated in the system, providing data freshness information.
9. `source`: Detailed information about where this data originates:

                    -`spreadsheet`: The name of the Excel file containing this data.
                    
                    -`sheet`: The specific worksheet within the spreadsheet where this cell is located.
                    
                    -`cell`:` The exact cell reference (e.g., "IS!M53") for pinpoint traceability.
